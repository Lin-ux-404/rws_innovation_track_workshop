...
def check_and_load_environment(required_vars):
    """Check if required environment variables are set and load .env file if needed."""
    ...
    if missing_vars:
        ...
        if still_missing:
            raise ValueError(
                f"Missing required environment variables: {', '.join(still_missing)}. "
                "Please create a .env file with these variables or set them in your environment."
            )
    
    return True

def display_environment_variables(mask_sensitive=True):
    """Display environment variables, optionally masking sensitive ones."""
    print("Environment Variables:")
    for key, value in os.environ.items():
        if mask_sensitive and any(sensitive in key.lower() for sensitive in 
                                ['key', 'secret', 'password', 'connection']):
            print(f"{key}: ******")
        else:
            print(f"{key}: {value}")